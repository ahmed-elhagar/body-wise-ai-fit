...
const CompactDailyView = ({
  ...
  const tCommon = tFrom('common');

  // Group meals by type
  const mealsByType = todaysMeals.reduce((acc, meal, index) => {
    const type = (meal.meal_type || (meal as any).type) || 'meal';
    ...
    return (
      <Card className="p-8 text-center bg-gradient-to-br from-white via-blue-50/30 to-green-50/50 border-0 shadow-xl backdrop-blur-sm">
        ...
        <p className="text-gray-600 mb-6 max-w-md mx-auto leading-relaxed">
          {String(tMealPlan('generateFirstPlan'))}
        </p>
            ...
            <div className={`flex items-center justify-between mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
              <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <h3 className="font-bold text-gray-800 text-lg capitalize">
                  {String(tCommon(mealType))}
                </h3>
              </div>
              <Badge variant="secondary" className="bg-blue-50 text-blue-200 text-blue-700">
                {meals.length} {meals.length === 1 ? String(tMealPlan('meal')) : String(tMealPlan('meals'))}
              </Badge>
            </div>
      ...
      <Card className="p-4 bg-gradient-to-r from-gray-50 to-blue-50 border-0 shadow-lg">
        <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
          <div className={`flex items-center gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
            <div className="flex items-center gap-2">
              ...
              <div className={`text-center ${isRTL ? 'text-right' : 'text-left'}`}>
                ...
                <div className="text-xs text-gray-600">{String(tCommon('calories'))}</div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <div className={`text-center ${isRTL ? 'text-right' : 'text-left'}`}>
                ...
                <div className="text-xs text-gray-600">{String(tCommon('protein'))}</div>
              </div>
            </div>
          </div>
          ...
          <div className={`flex items-center gap-2 text-sm text-gray-600 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <span>{todaysMeals.length} {String(tMealPlan('mealsPlanned'))}</span>
          </div>
        </div>
      </Card>
    </div>
