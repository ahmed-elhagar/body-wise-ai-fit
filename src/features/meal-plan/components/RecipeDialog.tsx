  ...
  const handleGenerateFullRecipe = async () => {
    ...
    try {
      ...
      if (updatedMeal) {
        ...
        onRecipeUpdated?.();
        toast.success(t('Recipe with image generated successfully!'));
      }
    } catch (error) {
        ...
        <div className="space-y-6">
          ...
          <div className={`flex flex-wrap gap-4 text-sm ${isRTL ? 'flex-row-reverse' : ''}`}>
            <div className={`flex items-center gap-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
              ...
              <span>{(currentMeal.prep_time || 0) + (currentMeal.cook_time || 0)} {t('minutes')}</span>
            </div>
            <div className={`flex items-center gap-1 ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          {!hasFullRecipe && (
            <div className="text-center py-4 bg-purple-50 rounded-lg border border-purple-200">
              <h3 className="font-semibold mb-2 text-purple-800">{t('Generate Complete Recipe')}</h3>
              <p className="text-sm text-purple-600 mb-4">
                {t('Get detailed ingredients, step-by-step instructions, and food image with AI')}
              </p>
              <Button
              ...
              >
                {isGeneratingRecipe ? (
                  <div className="flex items-center gap-2">
                    ...
                    <span>{t('Generating Recipe & Image...')}</span>
                  </div>
                ) : (
                  <>
                    <Sparkles className="w-4 h-4 mr-2" />
                    {t('Generate Complete Recipe')}
                  </>
                )}
              </Button>
            </div>
          ...
          {hasFullRecipe && currentMeal.ingredients && currentMeal.ingredients.length > 0 && (
            <div>
              <h3 className="font-semibold mb-3">{t('Ingredients')}</h3>
          ...
          {hasFullRecipe && currentMeal.alternatives && currentMeal.alternatives.length > 0 && (
            <div>
              <h3 className="font-semibold mb-3">{t('Alternative Ingredients')}</h3>
