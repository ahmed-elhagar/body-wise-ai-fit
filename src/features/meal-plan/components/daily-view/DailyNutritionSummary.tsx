  ...
  return (
    <Card className="p-3 bg-gradient-to-r from-blue-50 to-blue-100 border-0 shadow-lg rounded-lg">
      <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
        <div>
          <h2 className="font-bold text-gray-800 text-base mb-1">{t('mealPlan.todaysSummary')}</h2>
          <div className={`flex items-center gap-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
            <div className="flex items-center gap-1 text-sm text-gray-700">
              <Flame className="w-4 h-4 text-red-500" />
              <span className="font-semibold">{totalCalories}</span>
              <span className="text-xs">{t('common.calories')}</span>
            </div>
            <div className="flex items-center gap-1 text-sm text-gray-700">
              <Zap className="w-4 h-4 text-green-500" />
              <span className="font-semibold">{totalProtein}g</span>
              <span className="text-xs">{t('mealPlan.protein')}</span>
            </div>
          </div>
        </div>
