...
export const useEnhancedMealExchange = () => {
  ...
  const exchangeMeal = async (mealId: string, preferences: any) => {
    ...
    try {
      console.log('🔄 Exchanging meal with enhanced AI');
      ...
      if (error) {
        console.error('❌ Enhanced meal exchange error:', error);
        throw error;
      }

      if (data?.success) {
        console.log('✅ Enhanced meal exchange completed successfully');
      ...
      console.error('❌ Enhanced meal exchange failed:', error);
