...
export const MEAL_PLAN_PROMPTS = {
  SYSTEM_BASE: `You are a professional nutritionist. Generate EXACTLY 7 days starting from SATURDAY with meals totaling {totalMeals}. Each meal MUST have: type, name, calories (number), protein, carbs, fat, ingredients (array), instructions (array), prepTime, cookTime, servings. Return ONLY valid JSON - no markdown.`,
  
  SKELETON_ONLY: `Focus on ${nationality || 'international'} cuisine with realistic prep times ≤{maxPrepTime} minutes. Generate BASIC meal info only - detailed recipes will be fetched separately on-demand.`,
...
export const RECIPE_PROMPTS = {
  ...
  USER_TEMPLATE: `Create a detailed recipe for: {mealName}
...
export const EXERCISE_PROMPTS = {
  ...
  PROGRAM_TEMPLATE: `Design a {duration}-week program for:
Goal: {goal}
Fitness level: {level}
