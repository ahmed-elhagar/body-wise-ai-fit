...
const DayOverview = ({
  ...
  const mealTypeColors = {
    breakfast: 'bg-orange-50 border-orange-200 text-orange-700 hover:bg-orange-100',
    lunch: 'bg-green-50 border-green-200 text-green-700 hover:bg-green-100',
    dinner: 'bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100',
    snack1: 'bg-purple-50 border-purple-200 text-purple-700 hover:bg-purple-100',
    snack2: 'bg-pink-50 border-pink-200 text-pink-700 hover:bg-pink-100'
  };
      ...
      {dailyMeals.length > 0 ? (
        <div className="space-y-2">
          {dailyMeals.map((meal, index) => (
            <Card key={meal.id} className="bg-white border-fitness-primary-100 shadow-sm hover:shadow-md transition-all duration-300 rounded-lg overflow-hidden group">
              <div className="p-3">
                ...
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-start gap-2 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1 text-left">
                      <div className="flex items-center gap-2 mb-1">
                        <Badge className={`capitalize px-2 py-0.5 text-xs font-medium ${mealTypeColors[meal.meal_type as keyof typeof mealTypeColors] || 'bg-gray-50 border-gray-200 text-gray-700 hover:bg-gray-100'}`}>
                          {meal.meal_type}
                        </Badge>
                      ...
                      <h4 className="font-bold text-sm text-fitness-primary-800 leading-tight text-left">
                        {meal.name}
                      </h4>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-1 ml-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    <Button 
                      size="sm" 
                      onClick={() => onViewMeal(meal)}
                      className="h-6 w-6 p-0 bg-indigo-500 hover:bg-indigo-600 text-white rounded-md shadow-sm"
                    >
                    ...
                    <Button 
                      ...
                      className="h-6 w-6 p-0 bg-emerald-500 hover:bg-emerald-600 text-white rounded-md shadow-sm"
                    >
                ...
                <div className="grid grid-cols-3 gap-1.5 mt-2">
                  <div className="text-center bg-blue-50 rounded-md p-1.5 border border-blue-100">
                    <div className="text-xs font-bold text-blue-700">{meal.protein}g</div>
                    <div className="text-xs text-blue-600">Protein</div>
                  </div>
                  <div className="text-center bg-green-50 rounded-md p-1.5 border border-green-100">
                    <div className="text-xs font-bold text-green-700">{meal.carbs}g</div>
                  ...
                  <div className="text-center bg-orange-50 rounded-md p-1.5 border border-orange-100">
                    <div className="text-xs font-bold text-orange-700">{meal.fat}g</div>
