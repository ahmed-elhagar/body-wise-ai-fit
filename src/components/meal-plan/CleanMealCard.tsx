...
const CleanMealCard = memo<CleanMealCardProps>(({ 
  meal, 
  onShowRecipe, 
  onExchangeMeal, 
  isCompact = false 
}) => {
  const handleViewRecipe = React.useCallback(() => {
    onShowRecipe(meal);
  }, [onShowRecipe, meal]);

  const handleExchangeMeal = React.useCallback(() => {
    onExchangeMeal(meal);
  }, [onExchangeMeal, meal]);

  const totalTime = meal.prep_time + meal.cook_time;
  const servingText = meal.servings === 1 ? 'serving' : 'servings';
  ...
  return (
    <Card className="bg-white border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
      <CardContent className={isCompact ? "p-3" : "p-4"}>
        {/* Meal Header */}
        <div className="mb-3">
          <h3 className={`font-semibold text-gray-800 mb-1 ${isCompact ? 'text-sm' : ''}`}>
            {meal.name}
          </h3>
...
CleanMealCard.displayName = 'CleanMealCard';

export default CleanMealCard;
