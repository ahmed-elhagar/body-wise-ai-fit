...
import { CheckCircle2, Play, Clock, Dumbbell, Target, Youtube, ChevronDown, ChevronUp } from "lucide-react";
...
export const InteractiveExerciseCard = ({
  ...
  const handleSetComplete = () => {
    if (currentSet < (exercise.sets || 3)) {
      ...
      onExerciseProgressUpdate(exercise.id, currentSet, exercise.reps || "10", notes);
    } else {
        ...
        {isExpanded && (
          <div className="mt-4 space-y-4 animate-fade-in">
            ...
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-xl">
              <h4 className="font-semibold text-yellow-800 mb-2">Personal Notes</h4>
              <textarea
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                placeholder="Add your notes about this exercise..."
                className="w-full p-3 border border-yellow-300 rounded-lg bg-white resize-none focus:outline-none focus:ring-2 focus:ring-yellow-500"
                rows={2}
              />
            </div>
          </div>
