...
const ProgressOverview = () => {
  ...
  return (
    <Card className="relative overflow-hidden bg-gradient-to-br from-white via-purple-50 to-pink-50 border-0 shadow-xl rounded-2xl">
      {/* Animated Background */}
      <div 
      ...
      {/* Header */}
      <div className="relative bg-gradient-to-r from-purple-600 to-pink-600 p-4 text-white">
        <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          <Button 
            ...
            className="bg-white/10 hover:bg-white/20 text-white border border-white/30 backdrop-blur-sm rounded-lg transition-all duration-300 transform hover:scale-105 text-xs px-2 py-1"
          >
      ...
      <div className="relative p-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-3 border-2 border-blue-100">
              <div className={`flex items-center justify-between mb-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <h4 className="font-bold text-gray-800 text-sm flex items-center gap-2">
                  <Target className="w-4 h-4 text-blue-600" />
                  Weekly Goals
                </h4>
                <Badge className="bg-gradient-to-r from-blue-500 to-indigo-500 text-white border-0 text-xs px-2 py-0.5 rounded-lg shadow-lg">
              ...
              {/* Current Streak */}
              <div className="mt-3 bg-gradient-to-r from-emerald-400 to-green-500 rounded-lg p-3 text-white">
                <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={isRTL ? 'text-right' : 'text-left'}>
                    ...
                    <p className="text-lg font-bold">{progressData.streaks.current} days</p>
                  </div>
          ...
          {/* Recent Achievements */}
          <div className="space-y-3">
            <h4 className={`font-bold text-gray-800 text-sm flex items-center gap-2 ${isRTL ? 'text-right flex-row-reverse' : 'text-left'}`}>
            ...
            <div className="space-y-2">
              {progressData.achievements.map((achievement, index) => (
                <div key={index} className={`group relative bg-gradient-to-br from-white to-gray-50 rounded-xl p-3 border-2 border-gray-100 hover:border-purple-200 transition-all duration-300 transform hover:scale-102 hover:shadow-lg ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className={`relative flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <div className={`w-8 h-8 ${getAchievementStyle(achievement.type)} rounded-lg flex items-center justify-center text-sm border-2 shadow-lg transform group-hover:scale-110 transition-transform duration-300`}>
                      {achievement.icon}
                    </div>
                    <div className={`flex-1 ${isRTL ? 'text-right' : 'text-left'}`}>
                      <p className="font-bold text-gray-800 text-xs leading-tight group-hover:text-purple-700 transition-colors">
                        {achievement.title}
                      </p>
            ...
            <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg p-3 border-2 border-yellow-200">
              <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <Badge className="bg-gradient-to-r from-yellow-400 to-orange-500 text-white border-0 text-xs px-2 py-0.5 rounded-md shadow-lg">
                  15
                </Badge>
              </div>
            </div>
