        ...
        <div className={`flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 ${isRTL ? 'lg:flex-row-reverse' : ''}`}>
          <div className="flex-1 space-y-3">
            ...
            <div className="space-y-2">
              <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <h1 className={`text-2xl lg:text-3xl font-bold text-white leading-tight ${isRTL ? 'font-arabic' : ''}`}>
                  {getGreeting()}, {profile?.first_name || t('common:user')}!
                </h1>
              ...
              <p className={`text-white/85 text-base max-w-2xl leading-relaxed font-medium ${isRTL ? 'font-arabic' : ''}`}>
          ...
          <div className={`flex items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
            <div className="relative">
              ...
              {/* Status Indicator */}
              <div className={`absolute -bottom-1 w-4 h-4 bg-gradient-to-br from-green-400 to-emerald-500 rounded-full border-2 border-white flex items-center justify-center ${isRTL ? '-left-1' : '-right-1'}`}>
                <div className="w-1.5 h-1.5 bg-white rounded-full animate-pulse"></div>
              </div>
            </div>
            <div className={isRTL ? 'text-right' : 'text-left'}>
              <p className={`text-white font-bold text-base ${isRTL ? 'font-arabic' : ''}`}>
                {profile?.first_name || t('common:user')}
              </p>
              <p className={`text-white/80 text-sm ${isRTL ? 'font-arabic' : ''}`}>
                {t('dashboard:fitnessJourney')}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-3 gap-3">
          <div className="bg-white/15 backdrop-blur-sm rounded-xl p-3 border border-white/25 hover:bg-white/20 transition-all duration-300">
            <div className="text-center">
              ...
              <div className={`text-white/80 text-xs font-medium ${isRTL ? 'font-arabic' : ''}`}>
